...
void UpdatePad(void)
{
	...
	for ( i=0 ; i<padCount ; i++ ) 
	{
		...
		//* 十字上ボタン
		if (dijs.rgdwPOV[0] == 0)	padState[i] |= BUTTON_UPX;
		//* 十字下ボタン
		if (dijs.rgdwPOV[0] == 18000)	padState[i] |= BUTTON_DOWNX;
		//* 十字左ボタン
		if (dijs.rgdwPOV[0] == 27000)	padState[i] |= BUTTON_LEFTX;
		//* 十字右ボタン
		if (dijs.rgdwPOV[0] == 9000)	padState[i] |= BUTTON_RIGHTPX;
		// Trigger設定
