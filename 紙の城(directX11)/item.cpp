...
void UpdateItem(void)
{
	...
	for (int i = 0; i < ITEM_MAX; i++)
	{
		if (g_Item[i].use)
		{
			...
			auto p_Tile = GetMapInfo(g_Item[i].pos, { ITEM_HITBOX_WIDTH, ITEM_HITBOX_HEIGHT });

			//下が地面の場合
			for (int j = 0; j < 9; j++)
			{
				if ((p_Tile[j].attrib == MAP_ATTRIB_STOP) && (j==4 || j==5 || j == 6 || j == 7 || j == 8))
				{
					//地面に触れているかの処理
					if (CollisionRot(g_Item[i].pos.x, g_Item[i].pos.y + 1, p_Tile[j].pos.x, p_Tile[j].pos.y, ITEM_HITBOX_WIDTH, ITEM_HITBOX_HEIGHT, MAPCHIP_SIZE, MAPCHIP_SIZE, 0.0f))
					{
			...
			for (auto x : player)
			{
				...
				if (CollisionRot(g_Item[i].pos.x, g_Item[i].pos.y, x->GetPosition().x, x->GetPosition().y, ITEM_HITBOX_WIDTH, ITEM_HITBOX_HEIGHT, PLAYER_HITBOX_WIDTH, PLAYER_HITBOX_HEIGHT, 0.0f))
				{


					PlaySound(_SE_itemGet, 0, 0.3f);

					if (g_Item[i].type == ITEM_TYPE_PAPER2)  //しおりタイプ
					{
						int playerHP = x->GetOldHp();
						playerHP++;


						if (playerHP > x->GetMaxHp())
						{
							playerHP = x->GetMaxHp();
						}
						...
						x->SetHp(playerHP);
						//}
						//else if(g_Item[i].type == ITEM_TYPE_PAPER)  //巻物タイプ
						//{
						//	int playerItemCount = x->GetItemCount();
						//	playerItemCount + 10;
						//	x->SetItemCount(playerItemCount);
						//}
						//else 
						//{
						//	int playerItemCount = x->GetItemCount();  //紙屑タイプ
						//	playerItemCount++;
						//	x->SetItemCount(playerItemCount);
						//}
					}
