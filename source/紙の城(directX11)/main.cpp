...
HRESULT Init(HINSTANCE hInstance, HWND hWnd, BOOL bWindow)
{
	...
	SetSamplerState(FILTER_MODE_POINT, ADDRESS_MODE_MIRROR);//ドット絵なのでテクスチャのフィルターをOFF
...
D3DXVECTOR2 SetAnimation(int basePattern, int animeCount, int allWidthPattern, int allHeightPattern, int widthPattern, bool reverse)
{
	D3DXVECTOR2 uv;
	//U      W											N番目の % 横のパターン数
	uv.x = (1.0f / allWidthPattern) * ((animeCount) % widthPattern);	//横の座標計算
	//V     H											N番目の / 横の全パターン
	uv.y = (1.0f / allHeightPattern) * ((basePattern+ animeCount) / allWidthPattern); //縦の座標計算
	...
	if (reverse)
	{
		...
		//num = num / 100.0f;
		uv.x = 1 + (1 - uv.x) - Round(1.0f / allWidthPattern, 1000);//左端なので-0.25
	}
...
int RandomPercent(float percent1, float percent2, float percent3, float percent4, float percent5, float percent6, float percent7, float percent8, float percent9, float percent10)
{
	...
	float ran = GetRandomFloat(1, 100);
...
bool OffScreenJudge(float posX, float posY, float sizeX, float sizeY)
{
	...
	//上下判定
	if (posY + sizeY < 0 || posY - sizeY > SCREEN_HEIGHT * 3)judge = true;
