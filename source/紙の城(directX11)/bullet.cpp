...
void UpdateBullet(void)
{
	...
	for (int i = 0; i < BULLET_MAX; i++)
	{
		...
		D3DXVECTOR2 spd = g_Bullet[i].spd;
		//ベクトルの正規化
		D3DXVec2Normalize(&g_Bullet[i].spd, &g_Bullet[i].spd);
		g_Bullet[i].spd *= g_Bullet[i].sp;//目的のスピードにするためにスピードを乗算する
		//目的地の座標を算出
		g_Bullet[i].goalPos = g_Bullet[i].pos;
		g_Bullet[i].goalPos += g_Bullet[i].spd;
			...
			while (true)
			{
				...
				for (int j = 0; j < ENEMY_MAX; j++)
				{
					...
					if (
						...
						CollisionRot(g_Bullet[i].pos.x, g_Bullet[i].pos.y, p_Enemy.at(j)->GetPosition().x, p_Enemy.at(j)->GetPosition().y, BULLET_HITBOX_WIDTH, BULLET_HITBOX_HEIGHT, ENEMY_HITBOX_WIDTH, ENEMY_HITBOX_HEIGHT, g_Bullet[i].rot)
						)
					{
						...
						//敵のノックバック設定
						float degrees = 0;
						if (g_Bullet[i].spd.x < 0) degrees = 0;
						if (g_Bullet[i].spd.x > 0) degrees = 180;

						Spawner::Instance()->SetEnemyKnockBack(j, 20, degrees * 3.14159f / 180.0f);//ラジアンにπを足して反転

						//EnemyHpDown(j, 1);
						SetHitSpark(EFFECT::HIT_SPARK, g_Bullet[i].pos.x, g_Bullet[i].pos.y, 0);
				...
				spd *= 30;//ここのスピード分進んだ場合の当たり判定を行っていく
				g_Bullet[i].pos += spd;

				//目標地点を超えた場合
				if (
					//右上に弾が進んでいる場合
					(spd.x > 0 && spd.y < 0 && g_Bullet[i].goalPos.x < g_Bullet[i].pos.x && g_Bullet[i].goalPos.y > g_Bullet[i].pos.y) ||
					//右下に弾が進んでいる場合
					(spd.x > 0 && spd.y > 0 && g_Bullet[i].goalPos.x < g_Bullet[i].pos.x && g_Bullet[i].goalPos.y < g_Bullet[i].pos.y) ||
					//左下に弾が進んでいる場合
					(spd.x < 0 && spd.y > 0 && g_Bullet[i].goalPos.x > g_Bullet[i].pos.x && g_Bullet[i].goalPos.y < g_Bullet[i].pos.y) ||
...
void SetBullet(int type, float posX, float posY, float velX, float velY, float rot, float sp)
{
	for (int i = 0; i < BULLET_MAX; i++)
	{
		if (!g_Bullet[i].use)
		{
			...
			g_Bullet[i].spd = D3DXVECTOR2(velX, velY);
			g_Bullet[i].rot = rot;
			g_Bullet[i].sp = sp;
			g_Bullet[i].uv = D3DXVECTOR2(0, 0);
			g_Bullet[i].animePattern = 0;
...
void DeleteBullet(int index)
{
	...
	SetHitSpark(EFFECT::HIT_SPARK, g_Bullet[index].pos.x, g_Bullet[index].pos.y, 0);
}