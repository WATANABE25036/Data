...
//*****************************************************************************
int ModelLoadCou = 0;
std::unordered_map<std::string, DX11_MODEL> doubleCheckMap;
...
void LoadModel(char* FileName, DX11_MODEL* Model, std::string tag)
{
	...
	// 頂点バッファ生成
	{
		...
		bd.ByteWidth = sizeof(VERTEX_3D) * Model->Model.VertexNum;
		bd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		bd.CPUAccessFlags = 0;

		D3D11_SUBRESOURCE_DATA sd;
		ZeroMemory(&sd, sizeof(sd));
		sd.pSysMem = Model->Model.VertexArray;
	...
	// インデックスバッファ生成
	{
		...
		ZeroMemory(&bd, sizeof(bd));
		bd.Usage = D3D11_USAGE_DEFAULT;
		bd.ByteWidth = sizeof(unsigned short) * Model->Model.IndexNum;
		bd.BindFlags = D3D11_BIND_INDEX_BUFFER;
		bd.CPUAccessFlags = 0;

		D3D11_SUBRESOURCE_DATA sd;
		ZeroMemory(&sd, sizeof(sd));
		sd.pSysMem = Model->Model.IndexArray;
	...
	// サブセット設定
	{
		...
		for (unsigned short i = 0; i < Model->Model.SubsetNum; i++)
		{
			...
			D3DX11CreateShaderResourceViewFromFile(GetDevice(),
				Model->Model.SubsetArray[i].Material.TextureName,
				NULL,
				NULL,
...
{
	delete[] Model->Model.VertexArray;
	delete[] Model->Model.IndexArray;
	delete[] Model->Model.SubsetArray;
...
//=============================================================================
// モデルがすでに読み込まれていたらそれを渡す
//=============================================================================
bool DoubleCheck(const std::string tag, DX11_MODEL& Model)
{
	if (doubleCheckMap.count(tag))
	{
		//初期化済みのモデルデータを渡す
		Model = doubleCheckMap[tag];
		return false;
	}
