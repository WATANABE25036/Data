...
// マウスをセットする関数
void SetMouseWindow(float x, float y)
{
	//ウィンドウを取得
	HWND* hWnd = GethWnd();
	POINT cursorPos = { LONG(x), LONG(y) };

	ClientToScreen(*hWnd, &cursorPos);//指定した画面座標をクライアント領域の座標に変換

	SetCursorPos(cursorPos.x, cursorPos.y);
}

// マウスのウィンドウのクライアント内座標を取得する関数
POINT GetMouseWindow(void)
{
	//ウィンドウを取得
	HWND* hWnd = GethWnd();
	//マウス座標取得
...
// ウィンドウのクライアント外にカーソルがでないようにする
void WindowClipCursor()
{
	//ウィンドウを取得
	HWND* hWnd = GethWnd();
