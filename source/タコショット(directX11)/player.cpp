			...
			for (int j = 0; j < ITEM_MAX; j++)
			{
				...
				if (CollisionRot(g_Player[i].pos.x, g_Player[i].pos.y, p_Item[j].pos.x, p_Item[j].pos.y, PLAYER_HITBOX_WIDTH, PLAYER_HITBOX_HEIGHT, ITEM_WIDTH * 0.5f, ITEM_HEIGHT * 0.5f, 0.0f))
				{
					...
					case ITEM_TYPE_TREASURE_BOX:
						...
						{
							if (g_Player[i].coin < 25)
							{
								//コイン枚数表示
								SetDrawTextCreate("NOT ENOUGH COINS " + std::to_string(25 - g_Player[i].coin), SCREEN_WIDTH * 0.5f, SCREEN_HEIGHT * 0.5f, 4.0f, 1.0f, 90, 0.1f, 0.01f, 255.0f, 0.0f, 0.0f);
							}
							else
							{
								g_Player[i].coin -= 25;
								int ran = RandomPercent(30, 30, 40);
								GUN_TYPE gun;
								switch (ran)
								{
								case 1:
